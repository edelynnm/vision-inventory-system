  ...
  .post("/login", async (req, res) => {
    try {
      ...
      const { rows } = await pgClient.query(`
        SELECT user_id, user_role_id, user_business_id, fname, lname, email, password, is_verified
  ...
  .patch("/verify-email", async (req, res) => {
    try {
      ...
      const user = await pgClient.query(
        "UPDATE users SET is_verified = true WHERE user_id = $1 RETURNING user_role_id",
        [rows[0].user_id],
      );

      if (user.rows[0].user_role_id !== 1) {
        return res.json({
          success: true,
          type: "employee",
