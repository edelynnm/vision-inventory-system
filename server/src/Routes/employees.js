...
router
  ...
  .get("/", async (req, res) => {
    try {
      ...
      SELECT U.fname, U.lname, U.email, R.title, U.is_verified
      FROM users AS U
      INNER JOIN roles AS R ON R.id = U.role_id
      WHERE U.business_id = $1 AND U.role_id != 1
  ...
  .get("/roles", async (req, res) => {
    try {
      const { rows } = await pgClient.query("SELECT * FROM roles WHERE title != 'ADMIN'");
      return res.json(rows);
    } catch (error) {
      console.log(error);
      return res.sendStatus(500);
  ...
  .post("/register", async (req, res) => {
    try {
      ...
      await pgClient.query(
        `INSERT INTO users
        (business_id, role_id, email, password, fname, lname, verification_token)
      ...
      return res.json({ success: true, message: "Invitation sent successfully!" }); // invitation also acts as verification for non-admin users
    } catch (error) {
      console.log(error);
      return res.sendStatus(500);
    }
